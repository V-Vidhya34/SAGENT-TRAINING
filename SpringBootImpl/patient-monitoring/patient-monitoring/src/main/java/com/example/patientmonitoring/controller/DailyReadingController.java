...
public class DailyReadingController {
    ...
    public List<DailyReading> getByPatient(@PathVariable int patientId) {
        return dailyReadingService.getByPatient(patientId);
    }

    @GetMapping("/my/{patientId}")
    public List<DailyReading> getMyReadings(@PathVariable int patientId) {
        return dailyReadingService.getByPatientId(patientId);
    }
}
